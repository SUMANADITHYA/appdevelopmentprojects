...
//                                break;
                            ...
                            case 5:
                            	...
                            	break;
                            	// Inside the loginAccount method in the switch statement
                            case 6:
                                System.out.print("Enter the amount to withdraw: ");
                                int amountToWithdraw = Integer.parseInt(sc.readLine());
                                if (withdrawMoney(senderAc, amountToWithdraw)) {
                                    System.out.println("MSG: Amount withdrawn successfully!\n");
                                } else {
...
//    }
    ...
    public static boolean depositMoney(int acNo, int amount) throws SQLException {
        try {
            ...
            // Credit the amount to the account7
            sql = "UPDATE customer SET balance = balance + ? WHERE ac_no = ?";
            ps = con.prepareStatement(sql);
            ps.setInt(1, amount);
            ps.setInt(2, acNo);
...
//            ps.executeUpdate();
 ...
 // Add this method to the bankManagement class
    public static boolean withdrawMoney(int acNo, int amount) {
        try {
            // Validation
            if (acNo == NULL || amount <= 0) {
                System.out.println("Invalid Account Number or Amount!");
...
//}
//public class bankManagement implements AccountDetails{
//    private static final int NULL = 0;
//    private static final int DEFAULT_INITIAL_BALANCE = 1000;
//
//    static Connection con = connection.getConnection();
...
//    }
//    
//    public static void getBalance1(int acNo) {
//      try {
//          // Query
//          sql = "SELECT * FROM customer WHERE ac_no = ?";
//          PreparedStatement ps = con.prepareStatement(sql);
